...
%     opts.n (1,1) double {mustBeEqual(opts.n,maxBendingAngle)} = 1 % the number of notches
...
function mustBeEqual(a,b)
    % Test for equal size
    if ~isequal(a,length(b))
        eid = 'Size:notEqual';
        msg = 'Number of Notches must be equal to length of theta.';
        error(eid,msg)
    end
end
